...
$(document).on('click', '.block-living-filters .btn-filter', function(e){
  //e.preventDefault();
  var btn = $(this);
  btn.removeClass('changed');
  btn.toggleClass('active');
...
$(window).on('resize load', function(){
  var rows = $('.block-living-rows .living-rows'), inRow = 4, resolutionName = $('.visible-detect div').filter(':visible').data('resolution');
  if(resolutionName == 'xs' || resolutionName == 'sm') inRow = 2;
  rows.each(function(){
    var cells = $(this).find('.living-row'), i = 1, maxH = 0;
    cells.css({'min-height': 0});
    cells.each(function(){
      var cell = $(this), h = cell.outerHeight();
      cell.css({'min-height': 0}).addClass('not-done');
...
/*----------------------------------------------------*/
/*    Активируем кнопки "заказать" в списке экскурсий
/*----------------------------------------------------*/
$(document).ready(function(){

  "use strict";

  //var buttons = $(".btn-tour-order");
  $(".btn-tour-order").each(function(){
    console.log($(this));
